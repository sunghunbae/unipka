...
def test_sites():
    ...
    for mode in ['a2b', 'b2a']:
        res = microstate._protonate_template(smi='OC(O)=COCCN1CCN(C(c2ccccc2)c2ccc(Cl)cc2)CC1', mode=mode)
        print()
        print(mode)
        print(res)
        print()
...
def test_ensemble():
    ...
    num_microstates = 0
    for charge, _ in E.items():
        print(f"{charge} ({len(E)})")
        print(_)
        num_microstates += len(E)
    print()
    print(f"Total {num_microstates} microstates")
    return E
